...
-- =============================================================================
-- The primary function for defining authorization rules.
-- Parses the DSL, stores the rule, and generates views/triggers.
...
BEGIN
  ...
  LOOP
    CASE part->>'type'
      ...
      ELSE
        ...
        filters := filters || jsonb_build_array(part);
    END CASE;
  END LOOP;

  -- Store the rule (upsert)
  ...
  -- Clean up old generated objects for this rule
  DELETE FROM auth.generated_objects WHERE rule_id = rule_id;

  -- Generate SQL based on operation
  CASE operation
    WHEN 'select' THEN
      view_sql := auth._generate_select_view(p_table_name, columns, filters);
      EXECUTE view_sql;
      generated_sql := view_sql;

      -- Grant access
      EXECUTE format('GRANT SELECT ON api.%I TO authenticated', p_table_name);

      -- Track generated object
      INSERT INTO auth.generated_objects (rule_id, object_type, object_schema, object_name)
      VALUES (rule_id, 'view', 'api', p_table_name);

    WHEN 'insert' THEN
      -- Need the view to exist first for INSTEAD OF trigger
      IF NOT EXISTS (
        SELECT 1 FROM information_schema.views
        WHERE table_schema = 'api' AND table_name = p_table_name
      ) THEN
        RAISE EXCEPTION 'Must define SELECT rule before INSERT rule for table %', p_table_name;
      END IF;

      trigger_sql := auth._generate_insert_trigger(p_table_name, filters);
      EXECUTE trigger_sql;
      generated_sql := trigger_sql;

      EXECUTE format('GRANT INSERT ON api.%I TO authenticated', p_table_name);

      INSERT INTO auth.generated_objects (rule_id, object_type, object_schema, object_name)
      VALUES
        (rule_id, 'function', 'api', p_table_name || '_insert_trigger'),
    ...
    WHEN 'update' THEN
      ...
      trigger_sql := auth._generate_update_trigger(p_table_name, filters);
      EXECUTE trigger_sql;
      generated_sql := trigger_sql;
      ...
      INSERT INTO auth.generated_objects (rule_id, object_type, object_schema, object_name)
      VALUES
        (rule_id, 'function', 'api', p_table_name || '_update_trigger'),
    ...
    WHEN 'delete' THEN
      ...
      trigger_sql := auth._generate_delete_trigger(p_table_name, filters);
      EXECUTE trigger_sql;
      generated_sql := trigger_sql;
      ...
      INSERT INTO auth.generated_objects (rule_id, object_type, object_schema, object_name)
      VALUES
        (rule_id, 'function', 'api', p_table_name || '_delete_trigger'),
        (rule_id, 'trigger', 'api', p_table_name || '_delete');
  END CASE;

  RETURN format('Rule created: %s.%s (%s)', 'api', p_table_name, operation);
END;
$$;

COMMENT ON FUNCTION auth.rule(TEXT, VARIADIC JSONB[]) IS
  'Define an authorization rule for a table. Generates views and triggers.';

-- Helper to drop all generated objects for a table
...
BEGIN
  -- Drop triggers first
  FOR obj IN
    SELECT * FROM auth.generated_objects go
...
COMMENT ON FUNCTION auth.drop_rules(TEXT) IS
  'Drop all authorization rules and generated objects for a table.';
...
SECURITY DEFINER
AS $$
  SELECT table_name, operation, columns, filters, created_at, updated_at
  FROM auth.rules
  ORDER BY table_name, operation;
$$;

COMMENT ON FUNCTION auth.list_rules() IS
  'List all defined authorization rules.';
