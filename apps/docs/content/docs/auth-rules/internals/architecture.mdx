...
The `security_invoker = false` setting means the view runs with the owner's privileges (postgres), not the calling user's. This allows the view to read tables that `authenticated` users can't access directly.
...
These are marked `VOLATILE` to ensure PostgreSQL evaluates them after other WHERE conditions.
...
When you call `auth_rules.drop_rules('table_name')`, it uses this table to find and remove all generated objects.
