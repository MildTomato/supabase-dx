...
const KEYWORDS = new Set([
  'CREATE', 'TABLE', 'IF', 'NOT', 'EXISTS', 'PRIMARY', 'KEY', 'REFERENCES',
  'ON', 'DELETE', 'CASCADE', 'DEFAULT', 'UNIQUE', 'INDEX', 'USING',
  'ALTER', 'ADD', 'DROP', 'COLUMN', 'CONSTRAINT', 'FOREIGN',
  'INSERT', 'INTO', 'VALUES', 'UPDATE', 'SET', 'WHERE', 'SELECT', 'FROM',
  'ENABLE', 'ROW', 'LEVEL', 'SECURITY', 'POLICY', 'FOR', 'TO', 'WITH', 'CHECK',
  'FUNCTION', 'RETURNS', 'AS', 'BEGIN', 'END', 'RETURN', 'LANGUAGE',
  'TRIGGER', 'BEFORE', 'AFTER', 'EACH', 'EXECUTE', 'PROCEDURE',
  'GRANT', 'REVOKE', 'ALL', 'PRIVILEGES', 'ROLE', 'PUBLIC',
  'AND', 'OR', 'IN', 'IS', 'NULL', 'TRUE', 'FALSE', 'USING',
  'REPLACE', 'VIEW', 'MATERIALIZED', 'EXTENSION', 'SCHEMA',
  'TYPE', 'ENUM', 'DOMAIN', 'SEQUENCE', 'OWNED', 'BY',
  'NEW', 'OLD', 'DECLARE', 'RAISE', 'EXCEPTION', 'NOTICE',
]);
...
const TYPES = new Set([
  ...
  'JSON', 'JSONB', 'BYTEA', 'ARRAY', 'INTERVAL', 'MONEY', 'INET', 'CIDR',
  'PLPGSQL', 'SQL',
]);
...
function highlightToken(token: string, index: number): React.ReactNode {
  ...
  if (KEYWORDS.has(upper)) {
    return <Text key={index} color="blue">{token}</Text>;
  }
  
  // Types in cyan
  if (TYPES.has(upper)) {
    return <Text key={index} color="cyan">{token}</Text>;
  }
  
  // Strings in green
  if (token.startsWith("'") || token.startsWith('"')) {
    return <Text key={index} color="green">{token}</Text>;
  }
  
  // Comments in dim
  if (token.startsWith('--')) {
    return <Text key={index} dimColor>{token}</Text>;
  }
  
  // Numbers in yellow
  if (/^\d+$/.test(token)) {
    return <Text key={index} color="yellow">{token}</Text>;
  }
  ...
  if (/^\w+\(/.test(token)) {
    ...
    return <Text key={index}><Text color="magenta">{funcName}</Text>{rest}</Text>;
  }
...
function SqlLine({ line }: { line: string }) {
  ...
  return (
    <Text>
      {tokens.map((token, i) => highlightToken(token, i))}
    </Text>
  );
}
 ...
 */
export function SchemaFileDisplay({ path, content, maxLines = 10 }: { path: string; content: string; maxLines?: number }) {
  return (
