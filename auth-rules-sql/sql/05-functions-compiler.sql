...
-- Generate SELECT view from rule
...
-- Generate INSERT trigger from rule
...
BEGIN
  ...
  -- Build validation statements from filters
  FOR filter IN SELECT * FROM jsonb_array_elements(p_filters)
  LOOP
  ...
  IF NEW.%I IS DISTINCT FROM auth.uid() THEN
    RAISE EXCEPTION '%I must match authenticated user'
      USING ERRCODE = '42501';
  END IF;
$v$, col, col);
      ELSIF val_type = 'one_of' THEN
        validations := validations || format($v$
  IF NOT EXISTS (
    SELECT 1 FROM claims.%I
    WHERE user_id = auth.uid() AND %I = NEW.%I
  ) THEN
    RAISE EXCEPTION 'Not authorized for this %I'
      USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  IF v_count = 0 THEN
    RAISE EXCEPTION 'Record not found or not authorized'
      USING ERRCODE = 'P0002';
  END IF;
...
-- Generate DELETE trigger from rule
