...
import { startDevDb, stopDevDb, isPGliteAvailable, execOnDevDb, getDevDbUrl } from './pglite-dev.js';
...
function getAtlasBinary(): string {
  ...
  if (!existsSync(binaryPath)) {
    throw new Error(`Atlas binary not found at ${binaryPath}. Try reinstalling @ariga/atlas.`);
  }
...
export async function diffSchema(
  ...
  try {
    ...
    const localSchema = localFiles.map(f => f.content).join('\n').trim();
...
export async function inspectSchema(
  ...
  if (result.status !== 0) {
    throw new Error(sanitizeConnectionString(result.stderr || 'Schema inspect failed'));
  }
...
export async function applySchemaWithAtlas(
  ...
  const orderedSql = files.map(f => `-- File: ${f.path}\n${f.content}`).join('\n\n');
  ...
  try {
    ...
    console.error('[atlas] Target URL:', sanitizeConnectionString(connectionString));
    ...
    if (result.stdout) console.error('[atlas] stdout:', result.stdout.slice(0, 500));
    if (result.stderr) console.error('[atlas] stderr:', result.stderr.slice(0, 500));
    ...
    if (result.status !== 0) {
      return {
        success: false,
        output: sanitizeConnectionString(result.stderr || result.stdout || 'Atlas apply failed'),
      };
    }

    return {
      success: true,
      output: sanitizeConnectionString(result.stdout?.trim() || 'Schema applied'),
...
function isAlreadyExistsError(message: string): boolean {
  return ALREADY_EXISTS_CODES.some(code => message.includes(code)) ||
