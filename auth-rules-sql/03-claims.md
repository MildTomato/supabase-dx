...
2. System creates claims views (e.g., `claims.org_ids`)
3. Rules reference claims (e.g., `auth.one_of('org_ids')`)
...
```sql
CREATE VIEW claims.project_ids AS
...
```sql
CREATE VIEW claims.all_org_ids AS
...
auth.eq('org_id', auth.one_of('org_ids'))  -- documents rule
auth.eq('org_id', auth.one_of('org_ids'))  -- projects rule
auth.eq('org_id', auth.one_of('org_ids'))  -- settings rule
```
