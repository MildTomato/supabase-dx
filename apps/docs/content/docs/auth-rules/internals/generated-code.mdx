...
<Callout type="info">
The UPDATE trigger validates access via the WHERE clause. The actual
column updates are handled by PostgreSQL's INSTEAD OF trigger
mechanism, which passes the full NEW record back to the view.
</Callout>
