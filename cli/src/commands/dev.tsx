...
export async function devCommand(options: DevOptions): Promise<void> {
  ...
  console.log("");
  console.log(
    `${C.bold}supa dev${C.reset} ${C.secondary}— watching for schema and config changes${C.reset}`,
  );
  console.log("");
  console.log(
  ...
  console.log(
    `${C.secondary}Schema:${C.reset}  ${C.fileName}${relative(cwd, schemaDir)}${C.reset}`,
  );
  console.log(
    `${C.secondary}Config:${C.reset}  ${C.fileName}${relative(cwd, configPath)}${C.reset}`,
  );
  if (seedEnabled) {
    const seedDisplay =
      seedPaths.length === 1 ? seedPaths[0] : `${seedPaths.length} paths`;
    console.log(
      `${C.secondary}Seed:${C.reset}    ${C.fileName}${seedDisplay}${C.reset}`,
    );
  }
  console.log(
    `${C.secondary}Types:${C.reset}   ${C.value}every ${typesIntervalMs / 1000}s${C.reset}`,
  );
  if (options.dryRun) {
    console.log(
      `${C.warning}Mode:${C.reset}    ${C.warning}dry-run (no changes applied)${C.reset}`,
    );
  }
  console.log("");
  ...
  const applyConfigChanges = async () => {
    startSpinner(
      `${C.secondary}Syncing${C.reset} ${C.fileName}config.json${C.reset}`,
    );
    ...
    try {
      ...
      if (postgrestPayload && Object.keys(postgrestPayload).length > 0) {
        if (options.dryRun) {
          ...
          if (changedDiffs.length > 0) {
            log(
              `${C.secondary}  API config: ${changedDiffs.length} changes${C.reset}`,
            );
            for (const diff of changedDiffs.slice(0, 3)) {
              log(
                `${C.secondary}    ${diff.key}: ${String(diff.remote)} → ${String(diff.local)}${C.reset}`,
              );
            }
          }
        } else {
      ...
      if (authPayload && Object.keys(authPayload).length > 0) {
        if (options.dryRun) {
          ...
          if (changedDiffs.length > 0) {
            log(
              `${C.secondary}  Auth config: ${changedDiffs.length} changes${C.reset}`,
            );
            for (const diff of changedDiffs.slice(0, 3)) {
              log(
                `${C.secondary}    ${diff.key}: ${String(diff.remote)} → ${String(diff.local)}${C.reset}`,
  ...
  const applySchemaChanges = async (changedFiles: string[]) => {
    ...
    startSpinner(
      `${C.secondary}Syncing${C.reset} ${C.fileName}${fileList}${extra}${C.reset}`,
    );
    ...
    try {
      if (options.dryRun) {
        ...
        if (!diff.hasChanges) {
          ...
          stopSpinner(
            `${C.secondary}Would apply ${diff.statements.length} statements (dry-run)${C.reset}`,
          );
          for (const stmt of diff.statements.slice(0, 5)) {
            console.log(
              `  ${C.secondary}${stmt.length > 70 ? stmt.slice(0, 67) + "..." : stmt}${C.reset}`,
            );
          }
          if (diff.statements.length > 5) {
            console.log(
              `  ${C.secondary}... and ${diff.statements.length - 5} more${C.reset}`,
            );
          }
        }
      } else {
        ...
        if (result.success) {
          if (result.output === "No changes to apply") {
            ...
            stopSpinner(
              `Synced ${C.value}${result.statements ?? 0}${C.reset} schema statements`,
            );
            ...
            try {
              ...
              log(
                `${C.success}✓${C.reset} ${C.secondary}Types refreshed${C.reset}`,
              );
            } catch {
          ...
          stopSpinner(
            `${C.error}Schema sync failed: ${result.output}${C.reset}`,
            false,
          );
        }
      }
    ...
    try {
      ...
      if (result.success) {
        ...
        const errorSummary = result.errors
          ...
          .join(", ");
        stopSpinner(
          `${C.warning}Seeded with ${result.errors.length} errors: ${errorSummary}${C.reset}`,
          false,
        );
      }
  ...
  watcher.on("all", (event, filePath) => {
    ...
    const eventColor =
      event === "add" ? C.success : event === "unlink" ? C.error : C.icon;

    if (isConfig) {
      log(
        `${eventColor}${eventIcon}${C.reset} ${C.fileName}config.json${C.reset}`,
      );
      state.pendingConfigChange = true;
    } else if (isSeed) {
      const relPath = relative(seedDir, filePath);
      log(
        `${eventColor}${eventIcon}${C.reset} ${C.fileName}seeds/${relPath}${C.reset}`,
      );
      state.pendingSeedChange = true;
    } else {
      const relPath = relative(schemaDir, filePath);
      log(
        `${eventColor}${eventIcon}${C.reset} ${C.fileName}${relPath}${C.reset}`,
      );
      state.pendingSchemaChanges.add(relPath);
    }
  ...
  try {
    if (options.dryRun) {
      ...
      if (seedEnabled) {
        ...
        if (existingSeedFiles.length > 0) {
          log(
            `${C.secondary}Would apply ${existingSeedFiles.length} seed file(s)${C.reset}`,
          );
        }
      }
    } else {
      ...
      if (result.success) {
        if (result.output === "No changes to apply") {
          ...
          stopSpinner(
            `Synced ${C.value}${result.statements ?? 0}${C.reset} pending statements`,
          );
        ...
        stopSpinner(
          `${C.error}Initial sync failed: ${result.output}${C.reset}`,
          false,
        );
      }
    }
    ...
    stopSpinner(
      `${C.warning}Could not sync: ${error instanceof Error ? error.message : String(error)}${C.reset}`,
      false,
    );
  }
