 ...
 */
export function buildPostgrestPayload(config: ProjectConfig): UpdatePostgrestBody | null {
...
export function buildAuthPayload(config: ProjectConfig): UpdateAuthBody | null {
  ...
  if (auth.enable_signup !== undefined) payload.disable_signup = !auth.enable_signup;
  ...
  if (auth.refresh_token_reuse_interval !== undefined) {
    payload.security_refresh_token_reuse_interval = auth.refresh_token_reuse_interval;
  }

  // Email settings
  if (auth.email) {
    const email = auth.email;
    if (email.enable_signup !== undefined) payload.mailer_secure_email_change_enabled = email.enable_signup;
    if (email.double_confirm_changes !== undefined) payload.mailer_secure_email_change_enabled = email.double_confirm_changes;
    if (email.enable_confirmations !== undefined) payload.mailer_autoconfirm = !email.enable_confirmations;
    if (email.otp_length !== undefined) payload.mailer_otp_length = email.otp_length;
    if (email.otp_expiry !== undefined) payload.mailer_otp_exp = email.otp_expiry;
  ...
  if (auth.external) {
    for (const [provider, settings] of Object.entries(auth.external)) {
      ...
      if (settings.skip_nonce_check !== undefined) payload[`${prefix}_skip_nonce_check`] = settings.skip_nonce_check;
    }
  }
  ...
  if (auth.mfa) {
    const mfa = auth.mfa;
    if (mfa.max_enrolled_factors !== undefined) payload.mfa_max_enrolled_factors = mfa.max_enrolled_factors;
    
    if (mfa.totp) {
      if (mfa.totp.enroll_enabled !== undefined) payload.mfa_totp_enroll_enabled = mfa.totp.enroll_enabled;
      if (mfa.totp.verify_enabled !== undefined) payload.mfa_totp_verify_enabled = mfa.totp.verify_enabled;
    }
    
    if (mfa.phone) {
      if (mfa.phone.enroll_enabled !== undefined) payload.mfa_phone_enroll_enabled = mfa.phone.enroll_enabled;
      if (mfa.phone.verify_enabled !== undefined) payload.mfa_phone_verify_enabled = mfa.phone.verify_enabled;
      if (mfa.phone.otp_length !== undefined) payload.mfa_phone_otp_length = mfa.phone.otp_length;
      if (mfa.phone.template !== undefined) payload.mfa_phone_template = mfa.phone.template;
    }
  }
