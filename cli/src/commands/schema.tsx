...
function SchemaApp({ cwd, profileName, action }: SchemaAppProps) {
  ...
  async function runAction() {
    ...
    if (!isAtlasAvailable()) {
      setState({ step: 'error', error: 'Atlas not found. Run: pnpm add @ariga/atlas' });
      setTimeout(() => exit(), 100);
      return;
    }
    ...
    if (!existsSync(schemaDir)) {
      setState({ step: 'error', error: 'No supabase/schema directory found. Create schema files there.' });
      setTimeout(() => exit(), 100);
      return;
    }
    ...
    try {
      ...
      if (!dbPassword) {
        setState({ 
          step: 'error', 
          error: 'SUPABASE_DB_PASSWORD environment variable required for schema operations' 
        });
        setTimeout(() => exit(), 100);
        return;
      }
      ...
      const sessionPooler = poolerConfig.find((p: { database_type: string }) => p.database_type === 'PRIMARY');
      if (!sessionPooler?.connection_string) {
        setState({ step: 'error', error: 'Could not get database connection info' });
        setTimeout(() => exit(), 100);
        return;
      }
      const connectionString = sessionPooler.connection_string.replace('[YOUR-PASSWORD]', dbPassword);
      ...
      setState({
        step: 'error',
        error: error instanceof Error ? error.message : 'Schema operation failed',
      });
      setTimeout(() => exit(), 100);
    }
  }
  ...
  if (state.step === 'error') {
    return (
      <Box padding={1}>
        <Status type="error" message={state.error || 'Schema operation failed'} />
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Status 
        ...
        message={state.hasChanges ? "Schema changes detected" : "Schema is up to date"} 
      />
...
export async function schemaStatusCommand() {
  ...
  const { waitUntilExit } = render(
    <SchemaApp cwd={cwd} action="status" />
  );
