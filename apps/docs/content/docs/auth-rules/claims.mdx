...
A claim is a SQL query that maps users to the entities they belong
to — orgs, teams, folders, courses. It reads from your existing
tables and tells the system "this user is in these orgs" or "this
user is on these teams."

Claims don't grant access on their own.
[Rules](/docs/auth-rules/rules) reference claims to control what
users can read, create, update, and delete.

## Creating a claim

Define a claim with `auth_rules.claim()`. The SQL query must return
`user_id` as the first column:
...
SELECT auth_rules.claim('org_ids', $$
--                       ^^^^^^^^
--                       Name — referenced by rules later

  SELECT user_id, org_id FROM org_members
--       ^^^^^^^  ^^^^^^      ^^^^^^^^^^^
--       │        │           Source table (your existing data)
--       │        Value column (the entity)
--       Required first column
$$);
```

Now the system knows which org IDs belong to each user. Nothing is
enforced yet — [rules](/docs/auth-rules/rules) use this claim to
control what users can actually do.

## Using claims in rules

Reference a claim with `auth_rules.one_of()` inside an `eq()`
filter:
...
This means "only return documents where `org_id` is one of the
user's `org_ids`."

For more advanced scenarios — like filtering by a user's role
within a claim — use `in_claim()` with `check()`. See
[Role-based access](/docs/auth-rules/check-claim) for details.
...
**Keep claims simple** when possible. Complex JOINs and recursive
CTEs are fine, but simpler queries are faster.

**Changes apply immediately** — when you add a user to an org,
they can access that org's data right away. No cache to invalidate.
