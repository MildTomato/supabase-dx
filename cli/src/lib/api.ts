...
export class SupabaseClient {
  ...
  async getBranchDiff(branchRef: string, schemas = 'public'): Promise<string> {
    ...
    return this.request<string>('GET', `/v1/branches/${branchRef}/diff?${params}`);
  }
  ...
  async getFunction(projectRef: string, functionSlug: string): Promise<FunctionSlug> {
  ...
  // API Keys
  async getProjectApiKeys(projectRef: string, reveal = true): Promise<ApiKey[]> {
    const params = reveal ? '?reveal=true' : '';
    return this.request<ApiKey[]>('GET', `/v1/projects/${projectRef}/api-keys${params}`);
  }
  ...
  async applyMigration(
    ...
    await this.request('POST', `/v1/projects/${projectRef}/database/migrations`, {
      query,
      name,
      rollback,
    });
  ...
  async runReadOnlyQuery(projectRef: string, query: string): Promise<unknown> {
    return this.request('POST', `/v1/projects/${projectRef}/database/query/read-only`, {
      query,
    });
  }
  ...
  async getProjectHealth(
    ...
    return this.request(
      'GET',
      `/v1/projects/${projectRef}/health?${params}`
    );
  }
