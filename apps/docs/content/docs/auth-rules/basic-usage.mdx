---
title: Basic usage
description: Common authorization patterns
---

This page covers the most common authorization patterns you need. Each example
shows what you want to achieve and the rules to make it happen.

## User owns their data

The simplest pattern is when users can only see their own records. Use
`auth_rules.user_id()` to match the authenticated user:
...
If a user tries to create a document in an org they don't belong to, they get a clear error:
...
Unauthenticated users see only public documents. Authenticated users see public documents plus documents from their orgs.
...
| Public + private | `auth_rules.or_(eq('is_public', true), ...)` |

## Error codes

Write operations (insert, update, delete) return meaningful HTTP errors:

| Error | HTTP Status | Meaning |
|-------|-------------|---------|
| `P0002` | 404 | Record doesn't exist or you don't have access |
| `42501` | 403 | You don't have permission for this action |
