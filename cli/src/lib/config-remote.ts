 ...
 */
export function buildApiConfigFromRemote(remote: Record<string, unknown>): ApiConfig {
  const config: ApiConfig = {};
  
  if (remote.db_schema) {
    config.schemas = String(remote.db_schema).split(',').map(s => s.trim());
  }
  if (remote.db_extra_search_path) {
    config.extra_search_path = String(remote.db_extra_search_path).split(',').map(s => s.trim());
  }
 ...
 */
export function buildAuthConfigFromRemote(remote: Record<string, unknown>): AuthConfig {
  ...
  if (remote.uri_allow_list !== undefined && remote.uri_allow_list !== '') {
    config.additional_redirect_urls = String(remote.uri_allow_list).split(',').map(s => s.trim());
  }
  ...
  if (remote.external_anonymous_users_enabled !== undefined) {
    config.enable_anonymous_sign_ins = Boolean(remote.external_anonymous_users_enabled);
  }
